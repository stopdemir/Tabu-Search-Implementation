...
PopulationNumFunc<-function(solVector,electionMatrix){
  pop<-0;
  pop=sum(electionMatrix[nrow(electionMatrix),solVector])
...
funFeasibilityControl<-function(SolutionList,AdjacencyMatrix,ElectionMatrix,beta){
  ...
  ConKey=FALSE;
  for(i in 1:length(SolutionList)){
    if(funContiguityControl2(SolutionList[[i]],AdjacencyMatrix)==TRUE){
      Contotal=Contotal+1
    }
  }
...
#########################4th Step#########################################
###############extract one unit and add to the neighbour##################


funForthStepforInitialization<-function(SolutionList,adjMatrix,elecMatrix,beta){
  
  n<-length(SolutionList);
  averagePopulation<-sum(elecMatrix[nrow(elecMatrix),])/n
  t<-funFeasibilityControl(SolutionList,adjMatrix,elecMatrix,beta);
  iteration<-0;
  while(t[[3]]<31 && iteration <100 ){
    
    randomOne<-sample(1:length(SolutionList),1)
    iteration<-iteration+1;
    potentialOne<-funFindTheClustersThatAreNeighborToX(elecMatrix,SolutionList,SolutionList[[randomOne]],beta)
    PopofRandom<-sum(elecMatrix[nrow(elecMatrix),SolutionList[[randomOne]]])
    PopofPotential<-sum(elecMatrix[nrow(elecMatrix),potentialOne[[2]]])
    difference<-funDifferenceBetweenTwo(SolutionList[[randomOne]],potentialOne[[2]],SolutionList,elecMatrix)
    adjacentUnits<-funFindAdjacentUnits(SolutionList[[randomOne]],potentialOne[[2]],adjMatrix)
    theOnes<-c()
    whichKey<-0;
    if(PopofRandom>PopofPotential){
      whichKey<-1;
      for(i in 1:length(adjacentUnits)){
        theOnes<-append(theOnes,adjacentUnits[[i]][1])
      }
      theOnes<-unique(theOnes)
    }else{
      whichKey<-2;
      for(i in 1:length(adjacentUnits)){
        theOnes<-append(theOnes,adjacentUnits[[i]][2])
      }
      theOnes<-unique(theOnes)
    }
    ...
    if(whichKey==1){
      for(i in 1:length(theOnes)){
        d<-setdiff(SolutionList[[randomOne]],theOnes[i])
        p<-append(potentialOne[[2]],theOnes[i])
        scores<-append(scores,funDifferenceBetweenTwo(d,p,SolutionList,elecMatrix))
      }
    }else{
      for(i in 1:length(theOnes)){
        d<-append(SolutionList[[randomOne]],theOnes[i])
        p<-setdiff(potentialOne[[2]],theOnes[i])
        scores<-append(scores,funDifferenceBetweenTwo(d,p,SolutionList,elecMatrix))
      }
    }
    w<-c()
    for(i in 1:length(scores)){
      if(scores[i]>difference){
         w<-append(w,i)
      }
    }
    scores<-setdiff(scores,scores[w])
    theOnes<-setdiff(theOnes,theOnes[w])
    
    while(length(theOnes)>0){
      usedOrNotKey<-FALSE;
      minOneIndex<-which.min(scores)
      if(whichKey==1){
      d<-setdiff(SolutionList[[randomOne]],theOnes[minOneIndex])
      p<-append(SolutionList[[potentialOne[[1]]]],theOnes[minOneIndex])
      if(funContiguityControl2(d,adjMatrix)==TRUE && funContiguityControl2(p,adjMatrix)==TRUE){
      usedOrNotKey<-TRUE;
      SolutionList[[randomOne]]<-d
      SolutionList[[potentialOne[[1]]]]<-p
      }
      theOnes<-setdiff(theOnes,theOnes[minOneIndex])
      }else{
      d<-append(SolutionList[[randomOne]],theOnes[minOneIndex])
      p<-setdiff(SolutionList[[potentialOne[[1]]]],theOnes[minOneIndex])
      if(funContiguityControl2(d,adjMatrix)==TRUE && funContiguityControl2(p,adjMatrix)==TRUE){
      usedOrNotKey<-TRUE;
      SolutionList[[randomOne]]<-d
      SolutionList[[potentialOne[[1]]]]<-p
      ...
      #theOnes<-setdiff(theOnes,theOnes[minOneIndex])
      if(usedOrNotKey==TRUE){
        a<-scores[minOneIndex]
      }else{
        a<-difference;
      }
      ...
      scores<-setdiff(scores,scores[e])
      theOnes<-setdiff(theOnes,theOnes[e])
    }
  }
    t<-funFeasibilityControl(SolutionList,adjMatrix,elecMatrix,beta);
  }
...
funFindTheClustersThatAreNeighborToX<-function(electionMatrix,SolutionList,x,beta){
  adjacentListToX<-list()
  adjacentIndexVector<-c()
  adjacentMat<-funAdjacency()
  n<-length(SolutionList)
  ...
  Result_List<-append(Result_List,list(adjacentIndexVector[1]))
  Result_List<-append(Result_List,list(adjacentListToX[[1]]))
   #choose a random neighbor
  if(TRUE==FALSE){
  if(length(adjacentIndexVector)>1){
      ra<-sample(adjacentIndexVector,1)
      e<-list()
      e<-append(e,list(adjacentIndexVector[ra]))
      e<-append(e,list(adjacentListToX[[ra]]))
      Result_List<-append(Result_List,list(e))
  }
  else{
    e<-list()
    e<-append(e,list(adjacentIndexVector[1]))
    e<-append(e,list(adjacentListToX[[1]]))
    Result_List<-append(Result_List,list(e))
  }}
  return(Result_List)
...
funDifferenceBetweenTwo<-function(x,y,SolutionList,elecMatrix){
  
  PopofRandom<-sum(elecMatrix[nrow(elecMatrix),x])
  PopofPotential<-sum(elecMatrix[nrow(elecMatrix),y])
  averagePopulation<-sum(elecMatrix[nrow(elecMatrix),])/length(SolutionList)
  output<-(averagePopulation-PopofRandom)^2+(averagePopulation-PopofPotential)^2
  return(output)
